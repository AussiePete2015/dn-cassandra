...
With this static inventory file built, it's a relatively simple matter to deploy our ansible cluster using two `ansible-playbook` commands (one for the seed nodes and a second for the non-seed nodes). Examples of these commands are shown [here](Deployment-Scenarios.md).
